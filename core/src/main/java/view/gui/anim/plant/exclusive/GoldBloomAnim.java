...
public final class GoldBloomAnim {
    private GoldBloomAnim() {}

    public static void register(PlantAnimOverrides overrides) {
        overrides.register("Gold Bloom", GoldBloomAnim::resolve);
    }
    ...
    private static AnimPose resolve(PlantInstance plant, PamCatalog.PamEntry entry, PlantAnimRole role) {
        ...
        return switch (role) {
            ...
            case SPECIAL -> AnimPose.once(entry.path(), "attack", role);
            default -> null;
        };
    }
}
