...
public class StartupHelper {
	 ...
	 *                  Enabling this blocks the thread until the child JVM stops.
	 * @return whether a child JVM process was spawned or not.
	 */
	public static boolean startNewJvmIfRequired(boolean inheritIO) {
		...
		if (osName.contains("windows")) {
			// LWJGL3 extracts natives to java.io.tmpdir; non-ASCII user names can break that on Windows.
			// Use ProgramData instead and temporarily sanitize user.name while natives load.
	...
	private static final String LINUX_JRE_ERR_MSG =
			"A Java installation could not be found. If you are distributing this app with a bundled JRE,"
					+ " be sure to set the environment variable '__GL_THREADED_OPTIMIZATIONS'"
					+ " to '0'!";
	private static final String CHILD_LOOP_ERR_MSG =
			"The current JVM process is a spawned child JVM process, but StartupHelper has attempted"
					+ " to spawn another child JVM process! This is a broken state, and should not"
	 ...
	 * specified by {@link ProcessBuilder#environment()}; the same applies for
	 * {@link System#getProperties() System Properties}.
	 * @param isMac whether the current OS is macOS. If this is `false` then the current OS is assumed
	 *             to be Linux (and an immediate check for NVIDIA drivers is performed).
	 * @param inheritIO whether I/O should be inherited in the child JVM process.
	 *                  Enabling this blocks the thread until the child JVM stops.
	 * @return whether a child JVM process was spawned or not.
	 */
	public static boolean startNewJvm0(boolean isMac, boolean inheritIO) {
		...
		if (alreadyOnDesiredThread(isMac, processID)) {
			return false;
		}
		...
		List<String> jvmArgs = buildRestartArgs(isMac, processID);
		if (jvmArgs == null) {
			return false;
		}
		spawnRestartedJvm(jvmArgs, isMac, inheritIO);
		return true;
	}

	private static boolean alreadyOnDesiredThread(boolean isMac, long processID) {
		if (!isMac) {
			if (!isLinuxNvidia()) {
				return true;
			}
			return "0".equals(System.getenv("__GL_THREADED_OPTIMIZATIONS"));
		}
		...
		return "1".equals(System.getenv("JAVA_STARTED_ON_FIRST_THREAD_" + processID));
	}

	private static List<String> buildRestartArgs(boolean isMac, long processID) {
		...
		String mainClass = resolveMainClass(processID);
		if (mainClass == null) {
			return null;
		}
	...
	private static String resolveMainClass(long processID) {
	...
	private static void spawnRestartedJvm(List<String> jvmArgs, boolean isMac, boolean inheritIO) {
		try {
